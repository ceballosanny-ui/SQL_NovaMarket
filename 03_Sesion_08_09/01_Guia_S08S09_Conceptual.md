...
|---|---|
| ¿Cuántas filas retorna GROUP BY? ¿Por qué? | **Retorna 6 filas**, porque comprime y agrupa los 500 registros originales en las 6 ciudades (identificadores únicos) que existen. |
...
| Pregunta | Tu respuesta |
|---|---|
| ¿Qué CiudadID tiene Margen_Aproximado negativo? | **El 6 (o el correspondiente a Leticia)** |
| ¿Cuánto es esa pérdida? | **El valor negativo arrojado en el Margen_Aproximado para esa ciudad** |
...
| Pregunta | Tu respuesta |
|---|---|
| ¿Para decidir si cerrar Leticia, cuál usarías: SUM o AVG? | **Usaría SUM**. Para ver la rentabilidad global necesitamos el total (suma) de costos frente a ventas totales, no el promedio por transacción. |
...
| Pregunta | Tu respuesta |
|---|---|
| ¿Qué columna une las dos tablas? | **La columna común `CiudadID`** (`f.CiudadID = c.CiudadID`) |
| ¿Por qué ahora aparece 'Leticia' y no '6'? | **Porque el `JOIN` permitió ir a la tabla `DimCiudad` y traer el nombre real correspondiente a ese ID.** |
...
| ¿Cuánto es esa pérdida? | **El valor negativo total arrojado en Margen_Aproximado** |
